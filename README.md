...
今天的主要工作在于将之前的方法进行实现。具体地——

* 实现了`line_range`函数。下面的例子展示了这个函数的用法：
  ...
  '''
  line_range(s,'CONFIG_CC')
  [(3,4),(9,10)]
  ```

* 研究了一下HTML实现方法。我之前提到想要做成一个大的HTML，即所有的配置项都在里面。但是这中间麻烦的地方在于配置项有多个，而配置项对应的文件也有多个，如何组织是一个问题。

**明日工作**

* 继续研究可视化的内容。
* 并看看是否能够借助预处理的语法分析器，能够得到更加丰富的内容。

#### 7.19 

**今日工作**

今天的主要工作是阅读了Cpipmain的源码。具体地——

* 确定了大批量的可视化的思路：大致结构如下——
  ...
  │   │   ├── file_hash_1.html
  │   │   └── file_hash_2.html
  │   └── CONFIG_CC_IS_GCC.html
  └── index.html
  ```

  其中`index.html`是一个总的索引文件，里面是配置项的列表。而没有给配置项的索引都会重新定向到`config`下面的某个具体的文件下，例如`CONFIG_CC_IS_GCC.html`等。

* 尝试阅读源码了Cpipmain的源码，不过进展还不是特别顺利。这个任务虽然是我自己加的，但是我感觉比较有意义，如果是一个`json`文件的话，肉眼是看不清楚的，但是使用这样的`HTML`就比较一目了然，展示的效果也比较好。

**明日工作**

* 继续阅读Cpipmain的源码，希望能有突破的点

#### 7.20

**今日工作**

今天的工作主要是部分实现了文件的可视化功能。

* 首先确定了显示策略。这里遇见了一个难题，之前的分析，都是以配置项为单位，然后去搜索文件。即配置项-文件。而单个的文件可能有多个配置项包含其中。但是到了如果一个配置项搞一个文件显示，这样就比较麻烦。所以这里采用的策略和当时解决补丁用的策略相同，即使用单个的文件然后去做规则匹配，匹配到多个配置项。所以需要对之前的结果进行处理。最后建立起双向的映射，即文件-配置项和配置项文件的映射。

* 遇见了一点CSS的BUG。这方面我一直不擅长，在调试BUG。

**明日工作**

* 把BUG调试好，然后有一个展示结果，好写周报。